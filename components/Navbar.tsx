...
interface NavbarProps {
  onEditClick: () => void;
  onGoHome: () => void;
  onNavigate: (view: 'home' | 'bmc' | 'pitch-deck') => void;
  isAdmin: boolean;
}

const Navbar: React.FC<NavbarProps> = ({ onEditClick, onGoHome, onNavigate, isAdmin }) => {
  return (
      ...
      <div className="flex items-center gap-4">
        <button 
          ...
          className={`p-2 rounded-xl transition-all flex items-center gap-2 ${
            isAdmin ? 'bg-purple-600 text-white shadow-lg' : 'text-slate-700 hover:text-purple-600 bg-slate-50 hover:bg-purple-50'
          }`}
